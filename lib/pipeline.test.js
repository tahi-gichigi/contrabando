...
// =============================================
// pingHeartbeat() tests
// The heartbeat is the backstop that proves the bot did its job, so it must:
//  1) be DELIVERED, not fire-and-forget (an un-awaited fetch can be killed when
//     the serverless handler returns), and
//  2) distinguish SUCCESS from FAILURE — a healthy ping hits the base URL, a
//     failed run hits <url>/fail so Better Stack opens an incident. The handler
//     must NOT ping success unconditionally, or a crashing pipeline stays green.
// =============================================
console.log('\npingHeartbeat()');
...
async function runAsyncTests() {
  ...
  // No URL configured → no fetch attempted, resolves false.
  {
    ...
    const result = await pingHeartbeat('', { fetchImpl: fakeFetch });
    expect('skips when url is empty (returns false)', result, false);
    expect('does not call fetch when url is empty', called, false);
  }

  // Success ping → hits the base URL.
  {
    let seenUrl = null;
    const fakeFetch = (u) => { seenUrl = u; return Promise.resolve(); };
    await pingHeartbeat('https://uptime.example/hb/abc', { fetchImpl: fakeFetch });
    expect('success pings the base heartbeat url', seenUrl, 'https://uptime.example/hb/abc');
  }

  // Failure ping → hits <url>/fail so Better Stack opens an incident immediately.
  {
    let seenUrl = null;
    const fakeFetch = (u) => { seenUrl = u; return Promise.resolve(); };
    await pingHeartbeat('https://uptime.example/hb/abc', { fail: true, fetchImpl: fakeFetch });
    expect('failure pings the /fail endpoint', seenUrl, 'https://uptime.example/hb/abc/fail');
  }

  // /fail must not produce a double slash if the url has a trailing slash.
  {
    let seenUrl = null;
    const fakeFetch = (u) => { seenUrl = u; return Promise.resolve(); };
    await pingHeartbeat('https://uptime.example/hb/abc/', { fail: true, fetchImpl: fakeFetch });
    expect('failure handles trailing slash', seenUrl, 'https://uptime.example/hb/abc/fail');
  }
