...
// Ping a Better Stack heartbeat URL and AWAIT it so the request actually
// flushes before the serverless function freezes. A fire-and-forget fetch can
// be dropped on return, which Better Stack reads as a missed heartbeat (DOWN,
// then auto-resolve on the next ping). Errors are swallowed — monitoring must
// never break the cron. Returns true if a ping was attempted, false if no URL.
export async function pingHeartbeat(url, fetchImpl = fetch) {
  if (!url) return false;
  try {
    await fetchImpl(url);
  } catch {
    // Network blip pinging the monitor — ignore, the cron itself succeeded.
  }
  return true;
}
